...
module.exports = React.createClass({
  ...
  drag(e) {
    ...
    if(diffY + PADDING < 0) { // up
      var above = tree.getNodeByTop(index.top-1);
      newIndex = tree.move(index.id, above.id, 'before');
    } else if(diffY - PADDING > dragging.h) { // down
      if(index.next) {
    ...
    let hoveredNodeCollapseState = null
    if (this.state.hoveredNode !== hoveredNode) {

      clearTimeout(this.hoveredNodeTimeout)
      if (hoveredNodeCollapseState in this.state &&
        this.state.hoveredNodeCollapseState !== null) {
        this.state.hoveredNode.collapsed = this.state.hoveredNodeCollapseState
      }

      if (hoveredNode) {
        this.hoveredNodeTimeout = setTimeout(() => {
